module AddVolunteerPermissions_22043
open canopy
open runner
open etconfig
open lib
...
let core _ = 
    context "AddVolunteerPermissions_22043"
    compareTimeout <- 30.0
    once (fun _ -> portalLogin ())
    lastly (fun _ -> portalLogout ())
    let userName = getRandomString (20)
    let firstName = userName.[0..10]
    let lastName = userName.[5..19]
